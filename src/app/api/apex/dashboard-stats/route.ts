...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check if user is APEX role
    if ((session.user as any).role !== 'APEX') {
      return NextResponse.json({ error: 'Forbidden - APEX role required' }, { status: 403 });
    }
    ...
    console.error('APEX Dashboard stats error:', error);
