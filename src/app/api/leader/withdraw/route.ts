...
export async function POST(request: NextRequest) {
  try {
    // Check if withdrawals are enabled for LEADER role
    const withdrawalsEnabled = await isWithdrawalEnabled('LEADER');
    if (!withdrawalsEnabled) {
      return NextResponse.json({ 
        error: 'Withdrawals are currently disabled for leaders',
