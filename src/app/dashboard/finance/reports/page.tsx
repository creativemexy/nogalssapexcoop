...
export default function FinanceReportsPage() {
  ...
  const processTrendsData = () => {
    if (!reportData?.trends || !Array.isArray(reportData.trends)) return [];
    
    return reportData.trends
      .filter(trend => trend && trend.date && trend.amount !== undefined)
      .map(trend => ({
        ...
        formattedAmount: `₦${Number(trend.amount || 0).toLocaleString()}`
      }));
  };
  ...
  const getTotalPages = () => {
    if (!reportData?.transactions) return 0;
    return Math.ceil(reportData.transactions.length / pageSize);
  };
      ...
      <div className="h-full p-4">
        <div className="text-center mb-4">
          <h3 className="text-lg font-semibold text-gray-900">
            {type === 'line' ? 'Line Chart' : type === 'bar' ? 'Bar Chart' : 'Area Chart'}
          </h3>
        </div>
        <div className="relative h-64 border border-gray-200 rounded-lg p-4">
          {data.map((item, index) => {
            const height = range > 0 ? ((item.amount - minAmount) / range) * 200 : 100;
            const x = (index / (data.length - 1)) * 100;
            
            return (
              <div key={index} className="absolute bottom-0 flex flex-col items-center">
                <div 
                  className={`w-4 ${
                    type === 'line' ? 'bg-green-500 rounded-full' :
                    type === 'bar' ? 'bg-green-500' : 'bg-green-500'
                  }`}
                  style={{ 
                    height: `${height}px`,
                    left: `${x}%`,
...
This report was generated by the Nogalss Cooperative Finance System.
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {reportData?.databaseStatus === 'fallback' && (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-orange-700">
                ...
                Reports may not be up-to-date. Please check your database connection.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-sm text-gray-500">
              {reportData?.transactions.length || 0} total transactions
            </div>
          </div>
        </div>
          ...
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-700">
                Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, reportData.transactions.length)} of {reportData.transactions.length} transactions
              </span>
            </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-900">Financial Trends</h2>
            <div className="flex items-center space-x-4">
              ...
              >
                {useSimpleChart ? 'Simple Chart' : 'Advanced Chart'}
              </button>
              <div className="text-sm text-gray-500">
        ...
        {reportData.trends && reportData.trends.length > 0 ? (
          <div className="h-96">
            {useSimpleChart ? (
              <SimpleChart data={processTrendsData()} type={chartType} />
            ) : (
              <ChartErrorBoundary>
                {(() => {
                  try {
