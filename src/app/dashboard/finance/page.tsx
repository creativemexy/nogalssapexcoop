...
export default function FinancePage() {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch('/api/finance/notifications?limit=10');
      if (res.ok) {
        const data = await res.json();
        if (data.success) {
...
This report was generated by the Nogalss Cooperative Finance System.
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 sm:mb-8 space-y-4 sm:space-y-0">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-4">
          <button
            ...
            className="relative bg-purple-600 text-white px-3 sm:px-4 py-2 rounded hover:bg-purple-700 flex items-center space-x-2 text-sm"
          >
            <span>🔔</span>
          ...
          <Link
            ...
            className="bg-purple-600 text-white px-3 sm:px-6 py-2 rounded hover:bg-purple-700 text-sm"
          >
            <span className="hidden sm:inline">💸 Manage Withdrawals</span>
      ...
      {/* Charge Tracking Section */}
      <div className="bg-white rounded-lg shadow p-6 mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-gray-900">Transaction Charges</h3>
          <Link 
            href="/dashboard/super-admin/charge-tracking"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-orange-600">₦{stats.chargeTracking?.totalCharges?.toLocaleString() || '0'}</p>
            <p className="text-sm text-gray-500">Total Charges Recorded</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-purple-600">{stats.chargeTracking?.chargeCount || 0}</p>
            <p className="text-sm text-gray-500">Transaction Count</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-green-600">{stats.chargeTracking?.averageChargePercentage?.toFixed(2) || '0.00'}%</p>
            <p className="text-sm text-gray-500">Average Charge Rate</p>
          </div>
        </div>
      </div>
